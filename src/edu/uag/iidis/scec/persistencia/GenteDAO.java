...
public class GenteDAO {
    ...
    public void hazModificacion(Gente gente)
            throws ExcepcionInfraestructura {

        if (log.isDebugEnabled()) {
            log.debug(">hazModificar(gente)");
        }

        try {
            HibernateUtil.getSession().update(gente);
        } catch (HibernateException e) {
            if (log.isWarnEnabled()) {
                log.warn("<HibernateException");
            }
...
// de acuerdo al nuevo formato
    ...
    public Gente getByName(String nombreGente)
            throws ExcepcionInfraestructura {
        ...
        try {
            ...
            String hql = "from Gente where nombres = :nombres";
